...
import { blogStandardListData } from '../../data/blog/blogData';
import type { BlogStandardItem } from '../../data/blog/blogType';
                            ...
                            {currentBlog.map((blog: BlogStandardItem) => (
                                <div className="blog-list__single" key={blog?.id}>
                                    ...
                                    <div className="blog-list__content">
                                        ...
                                        <Link to={blog?.link} className="blog-list__read-more">Learn More<span className="icon-arrow-right"></span></Link>
                                    </div>
                                </div>
                            ))}
                            <div className="car-listing__pagination">
                                <ul className="pg-pagination list-unstyled">
                                    ...
                                    {Array.from({ length: totalPages }).map((_, index) => (
                                        <li key={index + 8} className={`count ${currentPage === index + 1 ? "active" : ""}`}>
